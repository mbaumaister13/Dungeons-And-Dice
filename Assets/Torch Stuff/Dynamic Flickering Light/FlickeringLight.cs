...
public class FlickeringLight : MonoBehaviour {
    ...
    void Start ()
    {
        ...
        intensityOrigin = lt.intensity;
        rangeOrigin = lt.range;
        ...
        intensityOffset = lt.intensity * scale;
        rangeOffset = lt.range * scale;
    ...
    private void IntensityAndRange()
    {
        ...
        {
            intensityDelta = (intensityOrigin + Random.Range(-intensityOffset, intensityOffset) - lt.intensity) * speed;


            rangeTarget = rangeOrigin + Random.Range(-rangeOffset, rangeOffset);
            rangeDelta = (rangeTarget - lt.range) * speed;

            setNewTargets = false;
        }

        lt.intensity += intensityDelta;
        lt.range += rangeDelta;


        if (Mathf.Abs(lt.range - rangeTarget) < 5f * scale) //////// CHECK IF TARGET IS NEAR ENOUGH. /////////
            setNewTargets = true;
